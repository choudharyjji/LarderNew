...
resPress = (resId,index) => {
 ...
 _handlePress() {
    if(this.state.oldpass!=null && this.state.newpass!=null && this.state.confpass!=null){
  if(this.state.newpass===this.state.confpass){
    if(this.state.oldpass===this.state.confpass){
     alert('Your old and new password is same. Please try another.')
     return
    }
  }
...
secureTextEntry={true} onChangeText={(text) => this.setState({oldpass:text})}/>
...
<Button
    containerStyle={{ height:50 ,marginTop:50,margin : 10, backgroundColor: '#90BA45', padding:15,bottom:0}}
    disabledContainerStyle={{backgroundColor: '#90BA45'}}
    style={{fontSize: 15, textAlign:'center',color: 'white', fontWeight:'bold'}}
