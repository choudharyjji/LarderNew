...
resPress = (resId,index) => {
 ...
 _renderItem = ({item,index}) => {
   ...
   return (
     ...
     <View style = {{height : 100 ,width : window.width ,flex :1 ,flexDirection :'row', marginLeft:10, marginRight:10, marginTop:10}} >
...
<View style={{flexDirection:'column', marginLeft:10, marginTop:10}}>
     <Text style = {{marginLeft :10 ,marginTop:5,color :'white',fontFamily :'TypoGraphica' ,fontSize :14}}>
   {item.product_name}
      </Text>
      <Text style = {{marginLeft :10 ,marginTop:5,color :'white' ,fontSize :14}}>
    Rs : {item.sale_price}
       </Text>
       <Text style = {{marginLeft :10 ,marginTop:5,color :'white',fontFamily :'TypoGraphica' ,fontSize :14}}>
 ...
 renderPage(image, index) {
  ...
  render() {
    var blogData =  GLOBAL.blogurl
var closing = `Created Date :${blogData.blog_date} `;;
...
<Text style = {{color :'white',fontSize : 16 ,marginLeft : 10, marginTop :19 ,width :width - 80}}>
{blogData.title}
</Text>

 </View>

<KeyboardAwareScrollView style = {{marginTop :5,backgroundColor:'black'}}>
<Image
         style={{width:width, height: 150}}
         source={{ uri: blogData.blog_image }}
       />
<Text style = {{margin : 10,color :'white',fontSize : 13 , fontWeight :'bold'}}>
{closing}
</Text>

<Text style = {{textAlign :'justify',margin : 10,color :'white',fontSize : 13 }}>
